...
"""
This file minifies the raw tweet files generated by a tweet hydrator.

This file is to convert and extract the relevant tweets from that other dataset...
...
- for each folder
    - for each file
        - extract file
        - open file and for each line
            - check if text contains our modified keywords
            - remove unneccesary items of tweet object
            - write to file in correct folder...
...
def main(dir, skip):
    ...
    # Get the source files, which are gz files...
    ...
    for compressed_file in compressed_files_list[:len(compressed_files_list)-skip]:
        uncompressed_output_file = compressed_file                                  \
                                        .replace(".jsonl.gz", ".jsonl.min.gz")           \
        if osp.exists(uncompressed_output_file):
            ...
            continue    # Because this file is already processed...
        line_count = raw_newline_count(compressed_file)
        ...
        with gzip.open(compressed_file, "r") as compressed_gzip_reader:
            with gzip.open(uncompressed_output_file, "w") as compressed_gzip_writer:
                #with tqdm(total=line_count,unit="lines",miniters=100) as pbar:
                for line in tqdm(compressed_gzip_reader, miniters=100,unit="lines"):
                    #pbar.update(1)
                    # Process the thingamazig...                    
                    try:
                        ...
                        # Good tweet. Remove uneeded components
