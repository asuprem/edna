...
class BaseStreamingIngest(BaseIngest, Iterator):
    ...
    def __next__(self) -> RecordCollection[StreamRecord]:
        ...
        # This block is never reached, because in SingleSourceSingleTargatTask, we 
        # process records one-at-a-time, and don't submit concurrent jobs to ingest_executor
        # So what end up happening is after the watermark is generated, the ingest never enters
        # the 'waiting for shutdown' stage. So this block is irrelevant, I think. Still, leave it
        # in case we need it for debugging. NOTE: remove this block when bugs are fixed
